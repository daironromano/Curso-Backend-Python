...
elegiveis = list(filter(lambda aluno: aluno["frequencia"] >= 75.0 and aluno["status_matricula"] == "ativo", alunos))

com_bonus = list(map(lambda aluno: {
    **aluno,
    "nota_final": min(aluno["nota_final"] * 1.10, 10.0) 
}, elegiveis))

destaques = list(filter(lambda aluno: aluno["nota_final"] >= 9.0, com_bonus))

print("Alunos em destaque:")
for aluno in destaques:
    print(f"- {aluno['nome']} | Nota final (com bônus): {aluno['nota_final']:.2f}")